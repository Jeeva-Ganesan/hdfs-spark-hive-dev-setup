...
configure_hadoop:
	...
	getent passwd yarn || useradd -g hadoop yarn
	getent passwd hdfs || useradd -g hadoop hdfs
	getent passwd mapred || useradd -g hadoop mapred
...
configure_spark:
	...
	i=$(grep -c 'spark:' /etc/passwd); i=$((i)); if [ "$i" == 0 ]; then useradd -g spk spark; fi
